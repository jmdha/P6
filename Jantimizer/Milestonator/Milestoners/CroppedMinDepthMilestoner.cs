...
{
    public class CroppedMinDepthMilestoner : BaseMilestoner
    {
        public CroppedMinDepthMilestoner(IDataGatherer dataGatherer, IDepthCalculator depthCalculator) : base(dataGatherer, depthCalculator)
        {
        }
        ...
        public override void AddMilestonesFromValueCount(TableAttribute attr, List<ValueCount> sorted)
        {
            ...
            while (groupQueue.Count > 0)
            {
                ...
                while (count < depth && groupQueue.Count() > 0 && (groupQueue.Peek().Count < depth))
                {
