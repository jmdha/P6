...
{
    [TestClass]
    public class MinDepthMilestonerTests
    {
        #region GenerateHistogram Table

        [TestMethod]
        //                    Row IDs: 1  2  3  4  5
        [DataRow("a", "v", new int[] { 1, 2, 3, 4, 5 }, 2, 4)]
        ...
        public void Can_AddMilestonesFromValueCount_Table(string tableName, string attrName, int[] counts, int depth, int expMilestoneCount)
        {
            ...
            IMilestoner milestoner = new MinDepthMilestoner(new TestDataGathere(), new ConstantDepth(depth));
